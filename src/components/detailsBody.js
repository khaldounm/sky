          ...
          <Grid item xs={12} className={classes.marginTopSM}>
            <Grid container spacing={4}>
              ...
              <Grid item xs={12} sm={8} md={8} lg={8}>
                <Typography className={`${classes.serviceSubTitle} ${classes.bold}`}>{details.title || details.name} ({(details.release_date || details.first_air_date || details.birthday).split('-')[0]})</Typography>
                <Typography className={`${classes.marginTopMD} ${classes.serviceTextBlack}`}>{details.genres ? details.genres.map((elem) => { return elem.name }).join(", ") : details.place_of_birth}</Typography>
                {type !== "person" ? <Typography className={`${classes.marginTopMD} ${classes.serviceTextBlack}`}>{details.runtime || details.episode_run_time[0]} minutes</Typography> : ''}
                ...
                <Typography component={"span"} className={`${classes.marginTopSM} ${classes.leftPaneGreyText}`}>
                  <Box position="relative" display="inline-flex" className={`${classes.marginTopSM} ${classes.blackBG} ${classes.round} ${classes.paddingAll}`}>
                    ...
                    >
                      <Typography className={`${classes.bold} ${classes.white}`} variant="caption" component="div" color="textSecondary">{`${Math.round(
                ...
                {type !== "person" ?
                  <>
                    ...
                    <Typography className={`${classes.marginTopMD} ${classes.serviceTextBlack}`}>{details.production_companies.map((elem) => { return elem.name }).join(", ")}</Typography>
                  </> : ''}
              </Grid>
            </Grid>
